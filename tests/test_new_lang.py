...
def test_add_new_lang_err_missing_site_name():
    ...
    result = build_overrided(custom_new_lang_alternate, PATHS)
...
def test_add_new_lang_err_missing_alternate():
    ...
    result = build_overrided(overrides, PATHS)
    assert result.exit_code == 1
    assert 'Languages in site_name and alternate do not match' in result.exception.args[0]
...
def test_add_new_lang_OK():
    ...
    Path(test_folder + '/docs-en').mkdir(exist_ok=True)
    Path(test_folder + '/docs-en/index.md').touch()
    Path(test_folder + '/docs-es').mkdir(exist_ok=True)
    Path(test_folder + '/docs-es/index.md').touch()
    Path(test_folder + '/docs-pt').mkdir(exist_ok=True)
    Path(test_folder + '/docs-pt/index.md').touch()
